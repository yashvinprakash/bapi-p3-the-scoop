...
function getOrCreateComments(url, request) // post comment.
{
  const requestComment = request.body && request.body.comment;
  const response = {};

  if(requestComment &&  database.users[requestComment.username]  && database.articles[requestComment.articleId] && requestComment.body )
  {
    const comment ={
...
function updateComment(url, request) // put comment.
{
  const id = Number(url.split('/').filter(segment => segment)[1]);
...
function deleteComment(url, request) // Delete Comment.
{
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const savedComment = database.comments[id];
  const response = {};
  ...
  if (savedComment) {
    ...
    const userArticleCommentIds = database.articles[articleId].commentIds;
    userArticleCommentIds.splice(userArticleCommentIds.indexOf(id), 1);
   //database.articles[savedComment.username].comments[id] = null;
...
function upvoteComment(url, request)// Upvote Comment
{
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const username = request.body && request.body.username;
...
function downvoteComment(url, request)// DownVote Comment
{
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const username = request.body && request.body.username;
  let  savedComment = database.comments[id];
